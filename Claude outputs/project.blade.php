...
@endphp
    ...
    <script>
        // FIX (bridge lokasi native Android): sebagian versi Android System
        // WebView menolak navigator.geolocation() di halaman HTTPS resmi dengan
        // pesan "Only secure origins are allowed" - keterbatasan WebView itu
        // sendiri, bukan masalah sertifikat/halaman. MainActivity.kt sisi app
        // menambahkan jembatan native window.AndroidLocation yang memanggil
        // LocationManager Android langsung (di luar navigator.geolocation
        // browser sama sekali) dan menyuntik hasilnya balik ke sini lewat dua
        // fungsi global di bawah. Saat dibuka lewat browser biasa (bukan app),
        // window.AndroidLocation tidak ada, jadi locate() otomatis tetap pakai
        // navigator.geolocation seperti biasa - tidak ada perubahan perilaku
        // untuk akses non-app.
        window.__androidLocationResult = function (lat, lng, accuracy) {
            const ctx = window.__dompisLocateCtx;
            if (!ctx) return;
            ctx.latitude = Number(lat).toFixed(7);
            ctx.longitude = Number(lng).toFixed(7);
            ctx.accuracy = Number(accuracy).toFixed(2);
            ctx.source = 'gps';
            ctx.locating = false;
            ctx.error = '';
        };
        window.__androidLocationError = function (code, message) {
            const ctx = window.__dompisLocateCtx;
            if (!ctx) return;
            const msgs = {
                1: 'Izin lokasi ditolak. Aktifkan izin Location untuk aplikasi ini di pengaturan.',
                2: 'Sinyal GPS tidak tersedia. Coba di area terbuka (dekat jendela/luar ruangan) lalu ulangi.',
                3: 'Pengambilan lokasi lebih dari 25 detik (timeout). Pastikan GPS aktif, coba lagi, atau isi manual.',
            };
            console.error('Native Android location error', code, message);
            ctx.error = msgs[code] || ('Lokasi gagal diambil (' + message + '). Coba lagi atau isi manual.');
            ctx.locating = false;
        };
    </script>
...
@else
    ...
    @elseif ($step === 3)
        <section class="mt-5 space-y-4">
            ...
            <div x-data="{ latitude: '{{ $state['survey']?->latitude }}', longitude: '{{ $state['survey']?->longitude }}', accuracy: '{{ $state['survey']?->accuracy }}', source: '{{ $state['survey']?->location_source ?? 'manual' }}', locating: false, error: '', locate() { this.locating = true; this.error = ''; window.__dompisLocateCtx = this; if (window.AndroidLocation && typeof window.AndroidLocation.requestLocation === 'function') { window.AndroidLocation.requestLocation(); return; } if (!navigator.geolocation) { this.error='GPS tidak didukung perangkat.'; this.locating=false; return; } navigator.geolocation.getCurrentPosition(p => { this.latitude=p.coords.latitude.toFixed(7); this.longitude=p.coords.longitude.toFixed(7); this.accuracy=p.coords.accuracy.toFixed(2); this.source='gps'; this.locating=false; }, (err) => { console.error('Geolocation error', err.code, err.message); const msgs = {1: 'Izin lokasi ditolak. Aktifkan izin Location untuk aplikasi ini di pengaturan.', 2: 'Sinyal GPS tidak tersedia. Coba di area terbuka (dekat jendela/luar ruangan) lalu ulangi.', 3: 'Pengambilan lokasi lebih dari 25 detik (timeout). Pastikan GPS aktif, coba lagi, atau isi manual.'}; this.error = msgs[err.code] || 'Lokasi gagal diambil. Coba lagi atau isi manual.'; this.locating=false; }, { enableHighAccuracy: true, timeout: 25000, maximumAge: 10000 }); } }"
