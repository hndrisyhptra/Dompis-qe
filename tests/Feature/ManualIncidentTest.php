...
class ManualIncidentTest extends TestCase
{
    ...
    public function test_service_builds_expected_format(): void
    {
        ...
        $incident = app(ManualIncidentService::class)->generate($branch, ProgramType::RECOVERY);

        // INP + id_branch + program(1) + DDMMYY + urut(01)
        $this->assertSame(sprintf('INP%d1020926%s', $branch->id_branch, '01'), $incident);

        Carbon::setTestNow();
    }

    public function test_sequence_increments_per_branch_program_and_date(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 9, 2, 10, 0, 0));
        $branch = Branch::create(['code' => 'SBY', 'name' => 'SURABAYA', 'region' => 'REGION JATIM']);
        ...
        $first = $service->generate($branch, ProgramType::RECOVERY);
        QeLop::create([
            'incident' => $first,
            'nama_lop' => 'LOP A',
        ...
        $second = $service->generate($branch, ProgramType::RECOVERY);
        $this->assertSame(substr($first, 0, -2).'02', $second);

        // Program berbeda -> urutan mulai dari 01 lagi.
        $preventive = $service->generate($branch, ProgramType::PREVENTIVE);
    ...
    public function test_endpoint_returns_generated_incident_using_login_branch(): void
    {
        ...
        $this->actingAs($admin)
            ->getJson(route('lop.manual-incident', ['program_type' => 'recovery']))
            ->assertOk()
            ->assertJson([
                'ok' => true,
    ...
    public function test_endpoint_validates_program_type(): void
    {
        $branch = Branch::create(['code' => 'SBY', 'name' => 'SURABAYA', 'region' => 'REGION JATIM']);
        $admin = $this->adminForBranch($branch);

        $this->actingAs($admin)
            ->getJson(route('lop.manual-incident', ['program_type' => 'bogus']))
            ->assertStatus(422);
