...
@section('title', 'Tipe Designator')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <h1 class="text-xl font-bold text-ink-900 dark:text-ink-50">Tipe Designator</h1>
            <p class="text-sm text-ink-500 dark:text-ink-400 mt-1">Jenis pekerjaan designator (Material, Jasa, Instalasi, ...). Kode <code class="text-xs">MATERIAL</code> dipakai reservasi material teknisi.</p>
        </div>
        <a href="{{ route('designator-types.create') }}">
            <x-button type="button" class="!w-auto px-4">+ Tambah Tipe</x-button>
        </a>
    </div>

    <form method="GET" action="{{ route('designator-types.index') }}" class="flex gap-3 mb-4">
    ...
    <x-table>
        ...
        <tbody class="divide-y divide-ink-100 dark:divide-ink-700">
            @forelse ($types as $type)
                <tr>
                    <td class="px-4 py-3 text-ink-900 dark:text-ink-50 font-medium">{{ $type->code }}</td>
                    <td class="px-4 py-3 text-ink-600 dark:text-ink-300">{{ $type->name }}</td>
                    <td class="px-4 py-3 text-ink-600 dark:text-ink-300">{{ $type->designators_count }}</td>
                    <td class="px-4 py-3">
                        <x-badge :variant="$type->is_active ? 'success' : 'neutral'">{{ $type->is_active ? 'Aktif' : 'Nonaktif' }}</x-badge>
                    </td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="{{ route('designator-types.edit', $type) }}" class="text-sm text-ink-600 dark:text-ink-300 hover:text-ink-900 dark:hover:text-ink-50 font-medium">Edit</a>
                        <span class="text-ink-300 dark:text-ink-600 mx-1">·</span>
                        <form method="POST" action="{{ route('designator-types.destroy', $type) }}" class="inline" onsubmit="return confirm('Hapus tipe ini?');">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="5" class="px-4 py-8 text-center text-ink-400 dark:text-ink-500 text-sm">Belum ada tipe.</td></tr>
            @endforelse
        </tbody>
    </x-table>
