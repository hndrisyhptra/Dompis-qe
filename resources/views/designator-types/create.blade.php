...
@section('title', 'Tambah Tipe Designator')

@section('content')
<div class="max-w-lg mx-auto">
    <div class="mb-6">
        <h1 class="text-xl font-bold text-ink-900 dark:text-ink-50">Tambah Tipe Designator</h1>
    </div>

    <x-card>
        <form method="POST" action="{{ route('designator-types.store') }}" class="space-y-5">
            @csrf

            <x-input name="code" label="Kode" placeholder="mis. INSTALASI" />
            <x-input name="name" label="Nama" placeholder="mis. Instalasi" />
