...
@section('content')
@php $reportRoute = 'reports.sisa-material'; @endphp
...
<div class="mx-auto max-w-7xl space-y-4 @if ($print) report-print @endif">
    ...
    <div class="no-print flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <p class="mt-1 max-w-xl text-sm text-ink-500 dark:text-ink-400">Selisih qty rencana dan qty aktual per LOP dan rekap per designator{{ $priced ? ', dinilai dengan '.$package?->name.'.' : '.' }}</p>
        </div>
