...
<div class="max-w-4xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <h1 class="text-xl font-bold text-ink-900 dark:text-ink-50">Region</h1>
            <p class="text-sm text-ink-500 dark:text-ink-400 mt-1">Wilayah operasional. Tiap branch berada di bawah satu region.</p>
        </div>
        <a href="{{ route('regions.create') }}">
            <x-button type="button" class="!w-auto px-4">+ Tambah Region</x-button>
        </a>
    </div>

    <form method="GET" action="{{ route('regions.index') }}" class="flex gap-3 mb-4">
        <input type="text" name="q" value="{{ $q }}" placeholder="Cari kode atau nama region..."
    ...
    <x-table>
        ...
        <tbody class="divide-y divide-ink-100 dark:divide-ink-700">
            @forelse ($regions as $region)
                <tr>
                    <td class="px-4 py-3 text-ink-900 dark:text-ink-50 font-medium">{{ $region->code }}</td>
                    <td class="px-4 py-3 text-ink-600 dark:text-ink-300">{{ $region->name }}</td>
                    <td class="px-4 py-3 text-ink-600 dark:text-ink-300">{{ $region->branches_count }}</td>
                    <td class="px-4 py-3">
                        <x-badge :variant="$region->is_active ? 'success' : 'neutral'">{{ $region->is_active ? 'Aktif' : 'Nonaktif' }}</x-badge>
                    </td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="{{ route('regions.edit', $region) }}" class="text-sm text-ink-600 dark:text-ink-300 hover:text-ink-900 dark:hover:text-ink-50 font-medium">Edit</a>
                        <span class="text-ink-300 dark:text-ink-600 mx-1">·</span>
                        <form method="POST" action="{{ route('regions.destroy', $region) }}" class="inline" onsubmit="return confirm('Hapus region ini?');">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="5" class="px-4 py-8 text-center text-ink-400 dark:text-ink-500 text-sm">Belum ada region.</td></tr>
            @endforelse
        </tbody>
    </x-table>
