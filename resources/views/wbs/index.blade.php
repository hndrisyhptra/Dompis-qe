...
<div class="max-w-5xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-extrabold tracking-tight text-ink-900 dark:text-white">Pemetaan WBS</h1>
        <p class="mt-1 text-sm text-ink-500 dark:text-ink-400">Sebaran LOP berdasarkan jenis WBS dan status pekerjaannya.</p>
    </div>
    ...
    @if ($canFilterLocation)
        <form method="GET" action="{{ route('wbs.index') }}" class="mb-6 grid gap-3 sm:grid-cols-[200px_200px_auto]">
            ...
            <div class="flex gap-2">
                ...
                @if ($regionFilter || $branchFilter)
                    <a href="{{ route('wbs.index') }}" class="grid min-h-10 place-items-center rounded-xl border border-ink-200 px-3 text-sm font-bold text-ink-500 dark:border-ink-700">Reset</a>
                @endif
            </div>
        </form>
    @endif
        ...
        @foreach ($wbsSummaries as $wbs)
            <div class="flex flex-col rounded-2xl border border-ink-100 bg-white p-5 shadow-sm dark:border-ink-800 dark:bg-ink-900">
                <div class="flex items-baseline justify-between gap-2">
                    <h2 class="font-bold text-ink-900 dark:text-white">{{ $wbs['label'] }}</h2>
                    <span class="text-2xl font-extrabold text-ink-900 dark:text-white">{{ $wbs['total'] }}</span>
                </div>
                <dl class="mt-4 grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                    @foreach ($wbs['buckets'] as $key => $count)
                        <div class="flex items-center justify-between">
                            <dt class="text-ink-500 dark:text-ink-400">{{ \App\Http\Controllers\WbsController::BUCKETS[$key]['label'] }}</dt>
                            <dd class="font-bold text-ink-800 dark:text-ink-200">{{ $count }}</dd>
                        </div>
                    @endforeach
                </dl>
                <a href="{{ route('wbs.show', [$wbs['slug'], ...$carry]) }}"
