...
@section('title', 'Edit Kategori Designator')

@section('content')
<div class="max-w-lg mx-auto">
    <div class="mb-6">
        <h1 class="text-xl font-bold text-ink-900 dark:text-ink-50">Edit Kategori Designator</h1>
        <p class="text-sm text-ink-500 dark:text-ink-400 mt-1">{{ $category->code }}</p>
    </div>

    <x-card>
        <form method="POST" action="{{ route('designator-categories.update', $category) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <x-input name="code" label="Kode" :value="$category->code" />
            <x-input name="name" label="Nama" :value="$category->name" />
            ...
            <x-active-select :value="$category->is_active" />

            <div class="flex items-center gap-3 pt-2">
                <x-button>Simpan Perubahan</x-button>
                <a href="{{ route('designator-categories.index') }}" class="text-sm text-ink-600 dark:text-ink-300 hover:text-ink-900 dark:hover:text-ink-50">Batal</a>
            </div>
        </form>
    </x-card>
</div>
