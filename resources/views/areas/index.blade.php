...
<div class="max-w-4xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <h1 class="text-xl font-bold text-ink-900 dark:text-ink-50">Area</h1>
            <p class="text-sm text-ink-500 dark:text-ink-400 mt-1">Area operasional — tiap region berada di bawah satu area (saat ini Area 3).</p>
        </div>
        <a href="{{ route('areas.create') }}">
            <x-button type="button" class="!w-auto px-4">+ Tambah Area</x-button>
        </a>
    </div>

    <form method="GET" action="{{ route('areas.index') }}" class="flex gap-3 mb-4">
        <input type="text" name="q" value="{{ $q }}" placeholder="Cari kode atau nama area..."
    ...
    <x-table>
        ...
        <tbody class="divide-y divide-ink-100 dark:divide-ink-700">
            @forelse ($areas as $area)
                <tr>
                    <td class="px-4 py-3 text-ink-900 dark:text-ink-50 font-medium">{{ $area->code }}</td>
                    <td class="px-4 py-3 text-ink-600 dark:text-ink-300">{{ $area->name }}</td>
                    <td class="px-4 py-3 text-ink-600 dark:text-ink-300">{{ $area->regions_count }}</td>
                    <td class="px-4 py-3">
                        <x-badge :variant="$area->is_active ? 'success' : 'neutral'">{{ $area->is_active ? 'Aktif' : 'Nonaktif' }}</x-badge>
                    </td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="{{ route('areas.edit', $area) }}" class="text-sm text-ink-600 dark:text-ink-300 hover:text-ink-900 dark:hover:text-ink-50 font-medium">Edit</a>
            ...
            @empty
                <tr><td colspan="5" class="px-4 py-8 text-center text-ink-400 dark:text-ink-500 text-sm">Belum ada area.</td></tr>
            @endforelse
        </tbody>
    </x-table>
